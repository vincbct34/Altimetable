...
import { eventTypeColors, eventTypeLabels } from "../lib/eventTypeStyles";
  ...
  return (
    <div className="planning-calendar">
      <div className="planning-calendar-toolbar">
        <ul className="planning-calendar-legend">
          {eventTypes.map((type) => (
            <li key={type}>
              <span
                className="planning-calendar-legend-swatch"
      ...
      {error && (
        <p className="planning-calendar-error">
          Could not load the planning: {error}
        </p>
      )}
      ...
      <Calendar
        ...
        tooltipAccessor={(event) =>
          `${event.title} (${eventTypeLabels[event.resource.type]})`
        }
      ...
      {events === null && !error && (
        <p className="planning-calendar-loading">Loading planning…</p>
      )}
        ...
        <EventForm
          ...
          onDelete={
            formState.mode === "edit"
              ? () =>
                  withWriteAccess((accessToken) =>
                    deleteEvent(accessToken, formState.event.id),
                  )
              : undefined
          }
        />
